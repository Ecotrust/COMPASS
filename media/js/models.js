function layerModel(options, parent) {
    ...
    self.addLegendToLayer = function() {
      ...
      } else if (self.type == "WMS" && (!self.legend || self.legend.length < 1)) {
...
function viewModel() {
    ...
    self.toggleFeatureLayer = function(show) {
      if(!show && !self.featureClick() && self.featureAttribution() && app.map.getLayersByName('Selected Features').length > 0) {
    ...
    */
    ...
    self.updateAggregatedAttributesOverlayWidthAndScrollbar = function() {
        setTimeout( function() {
            var overlayWidth = (document.getElementById('aggregated-attribute-content').clientWidth+75),
