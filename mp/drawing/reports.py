...
def get_summary_reports(grid_cells):
    ...
    attributes.append({'title': 'Number of Hexes', 'data': format(cell_count, ',d')})
    ...
    title = 'Intersected Ecoregions'
    unique_ecoregions = get_unique_values(grid_cells, 'ecoregion')
    data = str(unique_ecoregions)
    attributes.append({'title': title, 'data': data})

    title = 'Hex IDs'
    unique_ids = get_unique_values(grid_cells, 'unique_id')
    data = str(unique_ids)
    attributes.append({'title': title, 'data': data})

    title = 'TCD Mean'
    tcd_mean = get_average(grid_cells, 'tcd_mean')
    data = str(format_precision(tcd_mean, 4))
    attributes.append({'title': title, 'data': data})
