...
def tiles_page(request, slug=None, template='tiles_page.html'):
    ...
    import settings
    if 'EXTERNAL_PORT' in dir(settings):
        external_port = settings.EXTERNAL_PORT
    else:
        external_port = 80

    context = {'project_name': project_name, 'layer': layer, 'arctile_url': arctile_url, 'arcrest_url': arcrest_url, 'domain': get_domain(external_port)}
    return render(request, template, context)
